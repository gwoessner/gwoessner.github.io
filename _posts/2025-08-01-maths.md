...
<mark>Or $x^{-1}\cdot x = n$</mark>, par définition $x^{-1}$ est l'inverse de $x$, donc en les mélangeant on obtient la couleur neutre $n$.   
<mark>Donc on réécrit l'équation avec $n\cdot x = n$</mark>, on a juste remplacé $x^{-1}\cdot x$ par $n$ dans l'équation.  
<mark>Et $x=n$</mark>, car $n\cdot x = x$. 
...
On abstractionne / généralise Si on trouve des applications (imaginaires) tant mieux sinon tant pis
