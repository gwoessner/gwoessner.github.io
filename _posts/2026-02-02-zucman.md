...
- Parmi les taxes payées figurent les cotisations sociales. Or, celles-ci ne sont pas comparables à des impôts dans le sens où elles ouvrent des droits à des prestations futures. On les appelle d'ailleurs parfois du salaire différé[^a].
...
En résumé, lorsque cette méthodologie considère le taux de taxation des Français modestes, elle oublie de prendre en compte la majorité des revenus de ces Français, présents et futurs.
...
[^a]: Il faut d'ailleurs noter que lorsqu'on discute du poids de l'Etat sur les salaires, on fait souvent signaler que les cotisations sociales ne sont pas des impôts pour la même raison. Si c'est le cas du lecteur, pour être cohérent, il convient donc d'appliquer la même logique ici.
